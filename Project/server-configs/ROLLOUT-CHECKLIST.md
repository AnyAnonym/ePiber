...
- [ ] Empfaenger-/Personenname und -rolle, persoenlicher Betreff und Text, Ereignisdetail, Ergebnis, Akteur, Quittierungszeit, Zustellungen sowie Ereignis- und Meldungs-ID des Meldungsberichts erscheinen weder in Loki noch in Prometheus-Metriken oder -Labels.
...
- [ ] Zehn gemeinsame Dashboards sind vorhanden und fuer zugelassene Administratoren editier- und speicherbar; die Deploymentauswahl `live|paj` trennt Anwendungswerte eindeutig, Hostressourcen werden nicht doppelt gezaehlt. Die Fachdashboards `ePiber Meldungen` und `ePiber Hallenreservierung` sind vorhanden. Eine spaetere JSON-Neuinstallation darf UI-Aenderungen kontrolliert ueberschreiben; Datasources bleiben nicht editierbar.
- [ ] `ePiber Hallenreservierung` zeigt plausible Kapazitaets-, Fixplatz-, Wartelisten- und Historienwerte sowie Neuverteilungen und problematische Schreibausgaenge. Personen- und Batchwerte bleiben Loki-JSON-Felder; Zielpersonennamen und freie Payloads fehlen.
...
- [ ] Direkte lokale Berichtsproben gegen Live und PAJ sind nur von Loopback mit Bearer erfolgreich, liefern die richtige Deploymentkennung und akzeptieren den Optionsmodus beziehungsweise `from`/`to` mit maximal 31 Tagen und eine kontrollierte `selection`. Fehlender/falscher Bearer, ungueltige Auswahlen und nicht lokale Quellen werden abgewiesen; PK besitzt keinen Bericht. Der Token wurde dabei nicht in Shell-Historie, Prozessliste, Journal oder Protokoll offengelegt.
- [ ] `ePiber Meldungen` zeigt fuer Live und PAJ plausible Wiener Tagesreihen, Bereichssummen, persoenliche Empfaengerzahlen und eine gemeinsame Meldungsliste. Die Checkbox-Mehrfachauswahl bietet persoenliche Meldungen, alle und einzelne Bewerbshistorien sowie alle und einzelne Hallenrasterhistorien; `Alle` ist vorausgewaehlt und ueberlappende Gesamt-/Einzelauswahlen erzeugen keine Duplikate. Detailansicht und Exporte werden wie `messaging.sqlite` und `state.sqlite` geschuetzt.
