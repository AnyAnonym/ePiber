...
function inspectEventsReadOnly(filename, events = EVENTS) {
  ...
  try {
    ...
    const required = ["competition_events", "event_participants", "event_receipts", "event_deliveries", "event_ack_operations", "message_bulk_ack_operations", "messaging_revisions", "event_comments", "comment_reactions", "event_reactions", "event_interaction_operations", "competition_history_revision"];
    ...
    if (schemaVersion !== 12 || required.some((name) => !tables.has(name)) || !eventColumns.has("result")) fail("MESSAGING_SCHEMA_MISMATCH");
