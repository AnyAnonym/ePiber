...
test("HTTP-Session und WebSocket-Rollen funktionieren zusammen", async (t) => {
  ...
  const reportOptionsResponse = await fetch(`${httpBase}/internal/messaging-report?view=options`, { headers: { Authorization: `Bearer ${reportingToken}` } });
  assert.equal(reportOptionsResponse.status, 200);
  const reportOptions = await reportOptionsResponse.json();
  assert.equal(reportOptions.options.some(({ __text, __value }) => __text === "Historie: Cup" && __value === "competition:cup-1"), true);
  const combinedReportResponse = await fetch(`${reportUrl}&selection=all`, { headers: { Authorization: `Bearer ${reportingToken}` } });
  assert.equal(combinedReportResponse.status, 200);
  const combinedReport = await combinedReportResponse.json();
  assert.equal(combinedReport.totals.personalCount, 1);
  assert.equal(combinedReport.totals.competitionCount, 1);
  assert.equal(combinedReport.totals.totalCount, 2);
  assert.deepEqual(new Set(combinedReport.entries.map(({ area }) => area)), new Set(["Persönliche Meldung", "Bewerbshistorie"]));
  const invalidSelection = await fetch(`${reportUrl}&selection=competition%3Amissing`, { headers: { Authorization: `Bearer ${reportingToken}` } });
  assert.equal(invalidSelection.status, 400);
  assert.equal((await invalidSelection.json()).error.code, "REPORTING_SELECTION_INVALID");
